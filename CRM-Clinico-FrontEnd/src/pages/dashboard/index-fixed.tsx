...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const dashboardStats = await dashboardService.getEstadisticasGenerales();
        setStats(dashboardStats as DashboardStats);
        
        // Obtener citas de hoy
        const citasHoy = await citaService.obtenerCitas();
        setAppointments(citasHoy);
        
        // Obtener tareas pendientes
        const tareaResponse = await tareaService.obtenerTareas({ estado: 'pendiente', pagina: 1, porPagina: 5 });
        setTasks(tareaResponse.tareas);
  ...
  // Si no hay datos aún, mostrar mensaje
  if (!stats) {
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="info">No hay datos disponibles en este momento.</Alert>
      </Box>
    );
  }
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
                ...
                <Box>
                  <Typography variant="h5" fontWeight="bold" align="center">
                    {stats.nuevosClientesPorMes.length > 0 ? 
                ...
                <Box>
                  <Typography variant="h5" fontWeight="bold" align="center">
                    ${(stats.resumen.ingresosTotales / 1000).toFixed(1)}K
                  </Typography>
                  <Typography variant="body2" color="text.secondary" align="center">
                    Ingresos
                  </Typography>
